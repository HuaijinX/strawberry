...
Outline the approach took to clean and organize the data.
...
### Check if every line associated with a state and see which state has most rows
```{r}
#| label: examine rows
#| echo: false
...
cat("The state which has the most rows is",state_max)
...
### Start to clean the data, seperate the data by program: Census or Survey, see if all of the rows are accounted for
...
### Complete the cleaning of Census data which include:
### Separate composite columns 
I divide the 'Data Item' columns into 3 market types: 'Totals','Fresh Market', and 'Process Market' to seperately show whether they have sales. And I divide the Matric and make it a new column. At last, I delete all the NA values.
...
<!-- remove  -->
<!-- relocate columns -->
### result of dropping all the one value columns of data
```{r}
#| label: final cleanup
#| echo: false
...
### Clean the Value and CV columns

I clean the Value column by eliminate all the "," in the data and delete all the "(D)" values and make them into NA value. Then for the CV columns, I delete all the "(D)" and "(H)" and make them into NA value.


### result of Value and CV columns
...
val2<-strwb_census$`CV (%)`
val2<-gsub("(D)",NA,val2)
val2<-gsub("(H)",NA,val2)
strwb_census$`CV (%)` <- val2
...
strwb_survey$temp2a <- strwb_survey$temp2a |> str_replace( "MEASURED IN", "")

strwb_survey <- strwb_survey |>
    separate_wider_delim(  cols = temp3,
                           delim = " - ",
                           names = c("temp3a",
                                     "temp3b"),
                           too_many = "error",
...
strwb_survey$temp3a <- strwb_survey$temp3a |> str_replace( "MEASURED IN", "")
strwb_survey$temp4 <- strwb_survey$temp4 |> str_replace( "MEASURED IN", "")
strwb_survey <- strwb_survey |> unite(temp3b, temp1b, col="Sta", sep="")
strwb_survey <- strwb_survey |> unite(temp2b, Sta, col="Result", sep="")
strwb_survey$Result <-strwb_survey$Result |> str_replace("NA","")
strwb_survey$Result <-strwb_survey$Result |> str_replace("NA","")
